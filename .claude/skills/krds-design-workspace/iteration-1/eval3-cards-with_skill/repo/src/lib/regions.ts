/**
 * 법정동코드(행정표준코드) 기반 시/도-시/군/구 매핑 테이블.
 *
 * 한국관광공사 중심/집중률/연관 관광지 API(LocgoHubTarService1, TatsCnctrRateService,
 * TarRlteTarService1)는 TourAPI 자체 지역코드(areaCode2)가 아니라 이 법정동코드 체계의
 * areaCd(시/도 2자리)·signguCd(시/군/구 5자리)를 사용한다. 두 체계를 혼동하지 않도록 주의.
 *
 * 실제 API 호출로 검증된 값(예: areaCd=11, signguCd=11110 → 서울 종로구)을 기준으로 작성했으며,
 * 세종특별자치시는 하위 시/군/구가 없어 자기 자신의 코드를 signguCd로 사용한다.
 *
 * 군위군은 2023년 경상북도에서 대구광역시로 편입되어 27720으로 반영했다.
 */
...
export function findRegion(areaCd: string): Region | undefined {
  return REGIONS.find((r) => r.areaCd === areaCd);
}
...
export function findSigungu(
  ...
  return findRegion(areaCd)?.sigungu.find((s) => s.code === signguCd);
}
