...
- **Critical CSS-layering gotcha (do not "simplify" `globals.css` back to a plain import):**
  `krds-react/dist/index.css` ships an *unlayered* reset — `body,div,p,main,section,header,…{
  ...
  layer(krds);`) and pulls in Tailwind *without* preflight (`tailwindcss/theme.css` +
...
- **`tatsCnctrRatedList` (F2) has no per-spot filter parameter.** It only accepts `areaCd`/`signguCd`
  ...
  only when the candidate is unique (ambiguous → no match, to avoid showing a *different* spot's
